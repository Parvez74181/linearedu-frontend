  ...
  const renderCell = (item: any, columnKey: string | number) => {
    ...
    switch (columnKey) {
          ...
          <TableCell>
            <div className="flex items-center gap-1">
              {Array.from({ length: item.star }).map((_, i) => (
                <Star key={i} fill="#f59e0b" color="#f59e0b" />
              ))}
            </div>
          </TableCell>
        );
            ...
            {item.student ? (
              <>
                ...
                <User
                  ...
                  description={
                    item?.student?.email || item?.student?.studentsPhone
                  }
      ...
      case "updatedAt":
        return (
          <TableCell>
            {item?.updatedAt ? format(item.updatedAt, "dd MMM, yyyy") : "-"}
          </TableCell>
        );
      default:
