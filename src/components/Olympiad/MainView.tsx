...
const MainView = ({ fromPage, data, totalPage, totalRow, role }: Props) => {
  ...
  const renderCell = (item: any, columnKey: string | number) => {
    ...
    switch (columnKey) {
      ...
      case "startTime":
        return <TableCell>{item?.startTime ? format(item.startTime, "PPpp") : "-"}</TableCell>;
      case "endTime":
        return <TableCell>{item?.endTime ? format(item.endTime, "PPpp") : "-"}</TableCell>;
      case "updatedAt":
